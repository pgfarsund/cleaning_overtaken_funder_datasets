...
write.csv(litter_funcab_format, here("Clean/FUNDER_clean_plant_litter_decomposition.csv"), row.names = F)

litter <- read.csv(here("Clean/FUNDER_clean_plant_litter_decomposition.csv"))

litter %>% 
  ggplot(aes(x=treatment, y=rel_weight_loss)) + 
  geom_boxplot() +
  geom_jitter()
